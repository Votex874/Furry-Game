...
function Game() {
    ...
    this.hideVisibleFurry =  () => {
        const actuallDiv = document.querySelector(".furry");
    ...
    this.checkCoinCollision = function () {
        if(this.furry.x === this.coin.x && this.furry.y === this.coin.y){
            const actuallCoin = document.querySelector(".coin");
            actuallCoin.classList.remove("coin");

            this.score = this.score +1;
            const scoreHTML = document.querySelector("#score strong");
    ...
    this.gameOver = function () {
        if (this.furry.x < 0 || this.furry.y < 0 || this.furry.x > 9 || this.furry.y > 9) {
            ...
            const endDiv = document.querySelector('#over');
